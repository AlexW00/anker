...
export class CardCreationModal extends Modal {
	...
	private onSubmit: (fields: Record<string, string>, createAnother: boolean) => void;
	...
	constructor(
		...
		deckPath: string,
		onSubmit: (fields: Record<string, string>, createAnother: boolean) => void
	) {
		super(app);
	...
	onOpen() {
		...
		// Button container
		const buttonContainer = contentEl.createDiv({ cls: 'flashcard-modal-buttons' });
