...
export class DashboardView extends ItemView {
	...
	async render() {
		...
		// Header with toolbar
		const header = container.createDiv({ cls: 'flashcard-dashboard-header' });
		
		header.createEl('h2', { text: 'Flashcards' });
		
		const toolbar = header.createDiv({ cls: 'flashcard-dashboard-toolbar' });
		...
		if (decks.length === 0) {
			const emptyState = deckList.createDiv({ cls: 'flashcard-empty-state' });
			emptyState.createEl('p', { text: 'No flashcards yet.' });
			emptyState.createEl('p', { text: 'Create your first card to get started!' });
	...
	private renderDeckList(container: HTMLElement, decks: Deck[]) {
		...
		const renderDeck = (deck: Deck, depth: number = 0) => {
			...
			// Actions
			const actionsEl = deckEl.createDiv({ cls: 'flashcard-deck-actions' });
	...
	private startCardCreation() {
		...
		new DeckSelectorModal(
			...
			(deckResult) => {
				...
				void this.plugin.templateService.getTemplates(
					...
					if (templates.length === 0) {
						...
						new Notice(`No templates found in "${templateFolder}". Please create a template first.`);
						return;
					}
	...
	private showCardCreationModal(
		...
		new CardCreationModal(
			...
			(fields, createAnother) => {
				void this.plugin.cardService.createCard(
					deckPath,
					template.path,
					fields,
